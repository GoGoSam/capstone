...
/**
 * Reads the state from an Xbox 360 controller using JInput. This works on all
 * platforms. On Windows DirectInput will be used, which has the limitation that
 * the left and right triggers share the same axis.
 */
public class JInputXboxController extends XboxController {
    ...
    public boolean poll() {
        ...
        while (eventQueue.getNextEvent(event)) {
            ...
            if (id == Identifier.Axis.POV) {
                ...
                if ((diff & DPAD_RIGHT) != 0) {
                    notifyListeners(Button.right, (newDirection & DPAD_RIGHT) == DPAD_RIGHT);
                }
                if ((diff & DPAD_LEFT) != 0) {
                    notifyListeners(Button.left, (newDirection & DPAD_LEFT) == DPAD_LEFT);
                }
                if ((diff & DPAD_DOWN) != 0) {
                    notifyListeners(Button.down, (newDirection & DPAD_DOWN) == DPAD_DOWN);
                }
                if ((diff & DPAD_UP) != 0) {
                    notifyListeners(Button.up, (newDirection & DPAD_UP) == DPAD_UP);
                }
    ...
    public ControllerInput getLastInput() {
        if (lastButton != null) {
            return new ControllerInput(controller.getName(), lastButton);
        }
        if (lastAxis != null) {
            return new ControllerInput(controller.getName(), lastAxis);
        }
        return null;
    }
    ...
    static public class ControllerInput implements Input {
        ...
        public float getState() {
            ...
            if (axis != null) {
                return device.get(axis);
            }
            if (button != null) {
                return device.get(button) ? 1 : 0;
            }
            return 0;
        }
    ...
    static public List<XboxController> getJInputControllers() {
        ...
        for (Controller controller : ControllerEnvironment.getDefaultEnvironment().getControllers()) {
            if (controller.getType() == Controller.Type.GAMEPAD) {
                list.add(new JInputXboxController(controller));
            }
        }
        return list;
    }
